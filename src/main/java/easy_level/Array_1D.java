...
when we know exactly how many pieces of data we're going to have. For example, you might use an array to store
a list of student ID numbers, or the names of state capitals. To create an array of integers named myArray
that can hold four integer values, you would write the following code:

int[] myArray = new int[4];

This sets aside a block of memory that's capable of storing 4 integers. Each integer storage cell is assigned a unique
index ranging from 0 to one less than the size of the array, and each cell initially contains a 0. In the case of myArray,
we can store integers at indices 0, 1, 2, and 3. Let's say we wanted the last cell to store the number 12; to do this, we write:
...
2- Reads n integers corresponding to a0, a1, a2, a3, ..., a(n-1) from stdin and saves each integer ai to a variable, val.
...
each array element on a new line from left to right.
    ...
    public static void main(String[] args) {
        ...
        // Read n integers and save each integer ai to a variable, val
        for (int i = 0; i < n; i++) {
            a[i] = scan.nextInt();
        }
